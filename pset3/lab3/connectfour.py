...
class ConnectFourRunner(object):
    ...
    * The game ends when one of the two players has four consecutive tokens in a row
            ...
            for callback, id, symbol in ( player1, player2 ):
                ...
                while not has_moved:
                    try:
                        ...
                        print (f"Player {id} ({symbol}) puts a token in column {new_column}")
                        self._board = self._board.do_move(new_column)
                        has_moved = True
                    except InvalidMoveException as e:
                        if sys.stdout.encoding and 'UTF' in sys.stdout.encoding:
                            # print unicode(e)
                            print(str(e))  # Assuming e is a variable of type str (unicode in Python 2.x)
    ...
    def _do_gameend(self, winner):
        ...
        print (f"Win for {self._board.board_symbol_mapping[winner]}!")
        if sys.stdout.encoding and 'UTF' in sys.stdout.encoding:
            # print unicode(self._board)
            print(str(self._board))
        else:
...
if __name__ == "__main__":
    ...
    for i in range(2,5):
        boardAfterMoves = boardAfterMoves.do_move(i)
    print(f"Board after 3 moves : {boardAfterMoves}")
    boardAfterMoves = boardAfterMoves.do_move(5)
    boardAfterMoves = boardAfterMoves.do_move(3)
    boardAfterMoves = boardAfterMoves.do_move(6)
    boardAfterMoves = boardAfterMoves.do_move(1)
    boardAfterMoves = boardAfterMoves.do_move(6)
    boardAfterMoves = boardAfterMoves.do_move(0)

    print(f"Board after moves : {boardAfterMoves}")
    print(f"Longest Chain : {boardAfterMoves.longest_chain(boardAfterMoves.get_other_player_id())}")

    print(f"\nchain cells")
    print(boardAfterMoves.chain_cells(boardAfterMoves.get_other_player_id()))
    print(f"len of chain_cells: {len(boardAfterMoves.chain_cells(boardAfterMoves.get_other_player_id()))}")
    filteredChain = set(filter(lambda x : len(x) > 1, boardAfterMoves.chain_cells(boardAfterMoves.get_other_player_id())))
    print(f"filtered Chain : {filteredChain}, len : {len(filteredChain)}")
