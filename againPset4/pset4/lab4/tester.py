...
def show_result(testsummary, testcode, correct, got, expected, verbosity):
    ...
    if correct:
        if verbosity > 0:
            print (f"{testsummary}: Correct." )
        if verbosity > 1:
            print ('\t', testcode)
            print()
    else:
        print (f"{testsummary}: Incorrect.")
        print ('\t', testcode)
        print ("Got:     ", got)
...
def run_test(test, lab):
    ...
    if mytype == 'VALUE':
        ...
        raise Exception("Test Error: Unknown TYPE '%s'. Please make sure you have downloaded the latest version of the tester script. If you continue to see this error, contact a TA." % unknown_type)
...
#              for x in tests_module.__dict__.keys() if x[-8:] == "_getargs" ]
    ...
    for index, (testname, getargs, testanswer, expected, fn_name, type) in enumerate(tests):
        ...
        try:
            ...
            print (f"{index}: ({testname}: Function not yet implemented, NotImplementedError raised)")
            continue
        except Exception:
    ...
    print (f"Passed {ncorrect} of {ntests} tests.")
...
def get_target_upload_filedir():
    ...
    print()
    print (f"The default path is '{cwd}'")
    # target_dir = raw_input("[%s] >>> " % cwd)
    target_dir = input(f"[{cwd}] >>> ")
