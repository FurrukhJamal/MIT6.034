    ...
    for (test, train) in ((group1, group2), (group2, group1)):
        ...
        for legislator in test:
            ...
            if gold_standard == predicted:
                score += 1
                if verbose >= 2:
                    print (f" {legislator_info(legislator)}: {predicted} (correct)")
            else:
                if verbose >= 1:
                    print (f"* {legislator_info(legislator)}: got {predicted}, actually {gold_standard}")
    if verbose >= 1:
        print (f"Accuracy: {score}/{len(group1)+ len(group2)}")
    return score
...
class CongressIDTree(object):
    def __init__(self, legislators, vote_meanings, disorder_func=None):
        ...
        if homog_test:
            ...
            best_criterion = None
            # print(f"legislators[0]['votes'] : {legislators[0]['votes']}")
            # print(f"legislators[0] : {legislators[0]}")
            for vote_index in range(len(legislators[0]['votes'])):
                # print(f"vote_index : {vote_index}")
