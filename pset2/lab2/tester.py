...
def show_result(testsummary, testcode, correct, got, expected, verbosity):
    ...
    if correct:
        if verbosity > 0:
            print ("{testsummary}: Correct.")
        if verbosity > 1:
            print ('\t', testcode)
            print
    else:
        print ("{testsummary}: Incorrect.") 
        print ('\t', testcode)
        print ("Got:     ", got)
...
def run_test(test, lab):
    ...
    if mytype == 'VALUE':
        ...
        raise Exception(f"Test Error: Unknown TYPE '%s'.  Please make sure you have downloaded the latest version of the tester script.  If you continue to see this error, contact a TA.")
...
def test_online(verbosity=1):
    ...
    try:
        ...
        server = xmlrpc.client.Server(server_url, allow_none=None)
    ...
    server.submit_code(username, password, lab.__name__,  bytes(tarball_data, 'utf-8'))  #xmlrpclib.Binary(tarball_data))
