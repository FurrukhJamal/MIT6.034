...
try:
    ...
    # Importing set and frozenset types
...
def forward_chain(rules, data, apply_only_one=False, verbose=False):
    ...
    while old_data != set(data):
        old_data = set(data)
        for condition in rules:
            data = condition.apply(data, apply_only_one, verbose)
            if set(data) != old_data:
                break
...
def instantiate(template, values_dict):
    ...
    elif isinstance(template, str) or isinstance(template, bytes):
        return AIStringToPyTemplate(template) % values_dict
    else: raise ValueError(f"Don't know how to populate a {type(template)}")
...
class IF(object):
    ...
    def __init__(self, conditional, action = None, 
        ...
        # Deal with an edge case imposed by type_encode()
        if isinstance(conditional, list) and action is None:
