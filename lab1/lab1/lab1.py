...
male = IF(("male (?x)"),
        ...
        THEN("(?x) is a male", "self_identity (?x) (?x)"))

female = IF(("female (?x)"),
          THEN("(?x) is a female", "self_identity (?x) (?x)"), )



parent = IF(("parent (?x) (?y)"),
...
siblings = IF(AND("(?x) is a parent of (?y)",
                   "(?x) is a parent of (?z)",
                    NOT("self_identity (?y) (?z)")),
              THEN("(?y) and (?z) are siblings"))

brother = IF(AND("(?x) and (?y) are siblings", 
                  "(?x) is a male"),
            THEN("brother (?x) (?y)"))

# sister = IF(AND("(?x) and (?y) are siblings", 
#                   "(?x) is a female"),
#             THEN("sister (?x) (?y)"))

sister = IF(AND(OR("brother (?x) (?y)", "(?x) and (?y) are siblings"), 
                  "(?y) is a female"),
            THEN("sister (?y) (?x)"))


# son = IF(AND("(?x) is a parent of (?y)", 
#              "(?y) is a male"),THEN("son (?y) (?x)"))
...
father = IF(AND(
                OR("son (?x) (?y)", "daughter (?x) (?y)"), 
                "(?y) is a male", 
              ),
            THEN("father (?y) (?x)"))

daughter = IF(AND("(?x) is a parent of (?y)", 
             "(?y) is a female"),THEN("daughter (?y) (?x)"))

mother = IF(AND(OR("son (?x) (?y)", "daughter (?x) (?y)"), "(?y) is a female"),THEN("mother (?y) (?x)"))

cousin = IF(AND("(?x) is a parent of (?y)",
                "(?z) is a parent of (?k)",
                "(?x) and (?z) are siblings"),THEN("cousin (?y) (?k)"))


grandparent = IF(AND("(?x) is a parent of (?y)",
                     "(?y) is a parent of (?z)"),THEN("grandparent (?x) (?z)"))

grandchild = IF("grandparent (?x) (?z)",THEN("grandchild (?z) (?x)"))
 
# Then, put them together into a list in order, and call it
# family_rules.
family_rules = [male, female, parent, siblings,brother, sister, son, daughter, father, mother, cousin, grandparent, grandchild]                    # fill me in
...
HOW_MANY_HOURS_THIS_PSET_TOOK = '48'
WHAT_I_FOUND_INTERESTING = 'Implementing backchain with recurssion, was quiet a challenge'
WHAT_I_FOUND_BORING = 'implementing forward chain problem'
