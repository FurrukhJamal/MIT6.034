...
class Sum(list, Expression):
    ...
    def __repr__(self):
        ...
        return f"Sum({list.__repr__(self)})"
        # return f"Sum({self.__repr__()})"
...
class Product(list, Expression):
    ...
    def __repr__(self):
        ...
        return f"Product({list.__repr__(self)})"
...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        ...
        for term in expr1:
            for term2 in expr2:
                result.append(multiply(term, term2))
        return result
        ...
        result = Product([])
        for term in expr2:
            result.append(multiply(term, expr1))
        return result
...
if __name__ == "__main__":
    ...
    a = Sum([Sum([1, 2]), 3])
    print(a)
    print(f"simplified : {a.simplify()}")
    print("multiplication tests")

    b = Product([Product([2, 1]), 3])
    print(f"b = {b}")
    print(f"simplified : {b.simplify()}")
