...
class BinaryConstraint:
    ...
    def check(self, state, value_i=None, value_j=None):
        """
        state is the csp state and should be an instance of CSPState.
        value_i and value_j are the values assigned to variable i and j respectively. 
        If they are not provided, they are fetched from the state by looking up variable_i and variable_j's names.
        """
        ...
        if value_i is not None and value_j is not None:
            return self.check_func(value_i, value_j, self.var_i_name, self.var_j_name)
        else:
            raise Exception("Neither value_i nor value_j are set")

        # If values of i or j are not set, we really can't check this constraint. 
        # So the check passes.
        return True
...
class CSP:
    ...
    # Fixed
    def solve(self, constraint_checker=basic_constraint_checker, verbose=False):
        """
